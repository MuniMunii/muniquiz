...
import { ParticipateType } from "lib/validation/participate";
// jest.mock("nanoid", () => {
//   let counter = 0;
//   return {
...
const server=setupServer(
    ...
    const timer = new Date(
      ...
      expired_at: timer,
            ...
            return HttpResponse.json({message:'successfull fetching',data:reconstructQuiz})
        }
...
describe("useSessionQuiz Store Zustand", () => {
  ...
  it('Set Quiz into QuizState',async ()=>{
    ...
    const response=await fetch('/api/quiz/get-quizparam',{method:"POST",body:JSON.stringify({enterID:'ABC1234567'}),headers: { "Content-Type": "application/json" },})
  ...
  it('Toggle Answer in AnswerState Progress',async()=>{
    const response=await fetch('/api/quiz/get-quizparam',{method:"POST",body:JSON.stringify({enterID:'ABC1234567'}),headers: { "Content-Type": "application/json" },})
    const data=await response.json()
    const AnswerList=data.data.progress
    useSessionQuizStore.getState().addAnswer(AnswerList)
